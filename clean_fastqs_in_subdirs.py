...
for fname in se_fastq_files:
    ...
    with open(log_file, 'w') as log:
        ...
        decontaminated_fname = remove_single_read_contaminants(deadaptered_fname,
                                                               log_file_handle=log)
        log.write('\n')
...
for fname1, fname2 in pe_fastq_files:
    ...
    with open(log_file, 'w') as log:
        print 'Removing adapters'
        deadaptered_fname1, deadaptered_fname2 = trim_paired_read_adapters(fname1,
                                                                           fname2,
                                                                           log_file_handle=log)
        log.write('\n')
        print 'Removing contaminants'
        decontaminated_fname1, decontaminated_fname2 \
            = remove_paired_read_contaminants(deadaptered_fname1,
                                              deadaptered_fname2,
                                              log_file_handle=log)
        log.write('\n')
        print 'Removing low quality bases'
        trim_paired_read_low_quality_bases(decontaminated_fname1,
                                           decontaminated_fname2,
                                           log_file_handle=log)
