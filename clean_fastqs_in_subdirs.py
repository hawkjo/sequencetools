...
from low_quality_bases import bookend_qual_subread, trim_single_read_low_quality_bases,
                                trim_paired_read_low_quality_bases
...
for fname1, fname2 in pe_fastq_files:
    ...
    with open(log_file, 'w') as log:
        deadaptered_fname1, deapatered_fname2 \
                = trim_single_read_adapters(fname1, fname2, log_file_handle=log)
        log.write( '\n' )
        decontaminated_fname1, decontaminated_fname2 \
                = remove_single_read_contaminants(deadaptered_fname1, deadaptered_fname2, log_file_handle=log)
        log.write( '\n' )
        trim_single_read_low_quality_bases(decontaminated_fname1, decontaminated_fname2, log_file_handle=log)
